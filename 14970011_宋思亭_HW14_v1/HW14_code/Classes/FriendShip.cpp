...
void FriendShip::addSprite() {
  ...
  ship->setAnchorPoint(Point(0.5, 0));
...
void FriendShip::addPlayer() {
  ...
  player1->setPosition(Vec2(xpos, visibleSize.height));

  //设置人物出场粒子效果
	ParticleFireworks* firework = ParticleFireworks::create();
	firework->setDuration(1.0f);
	firework->setPosition(Vec2(xpos, 200));
	addChild(firework);
...
void FriendShip::onKeyReleased(EventKeyboard::KeyCode code, Event* event) {
  ...
  case cocos2d::EventKeyboard::KeyCode::KEY_RIGHT_ARROW:
	...
	if (IsPlayer1Hold) player1->setDisplayFrame(IdleWithBox1);
